...
**Yerel sürüm.** `mem.aserdargun.com` yalnızca planlanan alan adıdır. Üretim yayını, DNS ve diğer portföy depolarında değişiklik yapılmadı. Kavramsal komşular CTX, ARL, DPL, CUL, SEC ve EVL ile gerçek servis bağlantısı yoktur.
...
`npm run build` sonucu **dist/** yayımlanabilir statik pakettir. `public/staticwebapp.config.json` temel Azure Static Web Apps başlıklarını içerir; Azure kaynağı veya deployment yapılmaz. Alan adı doğrulanınca canonical/OG adresleri ve dağıtım iş akışı ayrıca eklenebilir. Üretim ortamına özel sonuç iddiası yoktur.
...
All six scenarios and three policies work locally without a model, API key, backend, account or vector database. Memory records, source references, revisions, scoped lexical retrieval, context budgets, corrections, conflicts, expiry, deletion, comparison and versioned JSON exports are functional. The planned domain is not a verified live service. The release artifact is `dist/`; no production deployment or DNS change is included.
