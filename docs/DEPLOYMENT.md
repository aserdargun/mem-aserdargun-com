...
- Custom domains: none. `mem.aserdargun.com` remains planned; no DNS changes.
...
4. Upload the prebuilt `dist` directory using the SHA-pinned official Azure deployment action. Azure source integration is not enabled and no second deployment workflow is generated.
...
For a manual live check, set `MEM_BASE_URL` to the exact Azure-generated HTTPS URL and run `npm run verify:live` and `npm run test:e2e`. The verifier defaults to local Git HEAD as the expected commit; GitHub Actions uses `GITHUB_SHA`.
